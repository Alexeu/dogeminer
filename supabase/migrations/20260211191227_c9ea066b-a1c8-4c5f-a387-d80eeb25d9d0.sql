...
DECLARE
  ...
  v_valentine_chars TEXT[][] := ARRAY[
    ['cupid-doge', 'Cupid Doge', '0.014950'],
    ['romeo-doge', 'Romeo Doge', '0.014950'],
    ['lovefairy-doge', 'Love Fairy Doge', '0.014950'],
    ['heartknight-doge', 'Heart Knight Doge', '0.014950'],
    ['lovequeen-doge', 'Love Queen Doge', '0.014950']
  ];
...
BEGIN
  ...
  CASE p_box_id
    WHEN 'common' THEN
      v_box_price := 1;
      v_drop_rates := '{"common": 100, "rare": 0, "epic": 0, "legendary": 0, "christmas": 0, "valentine": 0}'::jsonb;
    WHEN 'rare' THEN
      v_box_price := 4;
      v_drop_rates := '{"common": 40, "rare": 40, "epic": 20, "legendary": 0, "christmas": 0, "valentine": 0}'::jsonb;
    WHEN 'legendary' THEN
      v_box_price := 9;
      v_drop_rates := '{"common": 0, "rare": 40, "epic": 40, "legendary": 20, "christmas": 0, "valentine": 0}'::jsonb;
    WHEN 'christmas' THEN
      v_box_price := 15;
      v_drop_rates := '{"common": 0, "rare": 0, "epic": 30, "legendary": 40, "christmas": 30, "valentine": 0}'::jsonb;
    WHEN 'valentine' THEN
      v_box_price := 12;
      v_drop_rates := '{"common": 0, "rare": 0, "epic": 0, "legendary": 0, "christmas": 0, "valentine": 100}'::jsonb;
    WHEN 'supreme' THEN
      v_box_price := 20;
      v_drop_rates := '{"common": 0, "rare": 20, "epic": 50, "legendary": 30, "christmas": 0, "valentine": 0}'::jsonb;
    ELSE
  ...
  ELSE
    ...
    ELSE
      ...
      ELSE
        ...
        ELSE
          v_cumulative := v_cumulative + COALESCE((v_drop_rates->>'christmas')::numeric, 0);
          IF v_random_roll <= v_cumulative THEN
            v_selected_rarity := 'christmas';
          ELSE
            v_selected_rarity := 'valentine';
          END IF;
        END IF;
      END IF;
    END IF;
  ...
  CASE v_selected_rarity
    ...
    WHEN 'valentine' THEN
      v_character_roll := 1 + floor(random() * array_length(v_valentine_chars, 1))::integer;
      v_char_id_out := v_valentine_chars[v_character_roll][1];
      v_char_name_out := v_valentine_chars[v_character_roll][2];
      v_char_mining_rate := v_valentine_chars[v_character_roll][3]::numeric;
      v_char_rarity_out := 'valentine';
  END CASE;
