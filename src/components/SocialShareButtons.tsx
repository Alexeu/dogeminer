...
const SocialShareButtons = ({ referralLink, compact = false }: SocialShareButtonsProps) => {
  const { t } = useLanguage();
  
  const shareText = "🐕 ¡Gana DOGE gratis con RPG Doge! Colecciona personajes y mina Dogecoin 24/7. ¡Únete con mi enlace! 🚀";
  ...
  const handleShare = (url: string, name: string) => {
    ...
    toast.success(`Compartiendo en ${name}...`);
  };
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-medium text-muted-foreground">
        {t('referral.shareOn') || 'Comparte en redes sociales:'}
      </p>
      <div className="flex gap-2 flex-wrap">
        {shareLinks.map((link) => (
          <Button
            key={link.name}
            size="sm"
            className={`${link.color} text-white gap-2`}
            onClick={() => handleShare(link.url, link.name)}
          >
            {link.icon}
